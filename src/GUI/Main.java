 ...
 */
/*
 * LDC: Lista de cambios
 * - 23/5/2011: Se corrigió el bug de eliminación del código fuente, en
 *          compileMenuItemActionPerformed.
 * - 27/5/2011: Cambios menores. Se reacomodaron los comentarios para dejar
 *          la corrección "accesible".
 */
...
public class Main extends javax.swing.JFrame {
    ...
    private void runMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_runMenuItemActionPerformed
        ...
        compileMenuItem.setEnabled(false);
        buttonCompile.setEnabled(false);
        String sourcePath = desktopPane.getSelectedFrame().getTitle();
            File src = new File(sourcePath);
            String tamName;
            try {
                tamName = src.getName().substring(0, src.getName().lastIndexOf(".")) + ".tam";
            } catch (Exception e) {
                // El nombre no tiene un punto
                tamName = src.getName() + ".tam";
            } 
        interpreter.Run(tamName);
...
// </editor-fold>
    ...
    private void compileMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_compileMenuItemActionPerformed
        ...
        if (((FileFrame)desktopPane.getSelectedFrame()).getPreviouslySaved()) {
            ...
            /*
             * DONE: Este código fue agregado para corregir el bug de que
             * el IDE eliminaba el archivo si tenía una extensión distinta
             * a ".tri" (minúsculas).
             * De aquí en adelante también se refactorizaron algunas variables.
             */
            String sourcePath = desktopPane.getSelectedFrame().getTitle();
            File src = new File(sourcePath);
            String tamName;
            ...
            File bytecodeFile = new File(src.getParent(), tamName);
            String bytecodePath = bytecodeFile.getAbsolutePath();
            bytecodeFile.delete();
            
            output.setDelegate(delegateConsole);            
            if (compiler.compileProgram(sourcePath)) {           
                output.setDelegate(delegateTAMCode);
                //disassembler.Disassemble(bytecodePath);
    ...
    // <editor-fold defaultstate="collapsed" desc="More Event Handlers Implementation ">
    /**
